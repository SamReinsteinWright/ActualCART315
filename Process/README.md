...
## tying up loose ends
This week was a busy one. my goals for this week were to implement enemies, as well as as a way to control the sheep, expand the play area and allow interactions between bullets and enemies. I started with attempting to figure out how to control the sheep in just an obtuse enough manner well also fitting with what I’ve learned so far. I found it difficult to program directly the sheep moving away from the mouse, so I came up with another solution. When the mouse enters the sheep’s collision detection View hit box a function is passed in which it checks the mouse’s current velocity and adds that to the sheep’s, so you swipe across the sheep with the mouse to move them. I thought this could be an interesting mechanic as the way it makes your mouse end up moving resembles that of a herding dog, running around the flock. I then moved on to creating the enemies. I started by creating a new prefab based on the sheep and copied over all the code from the sheep, and then deleted most of the code and replaced it with a code that detects when another object enters its area and if that object is a sheep, it will move towards it. That’s about everything the enemy does. In the sheep script, I added a part which detects if an enemy gets too close and then destroys the sheep if so. As for the bullets, I just detected on collision, whether or not it was colliding with an enemy and to destroy both the bullet and the enemy on contact, I had to put the bullet and the sheep on separate layers of collision to make sure that there were no incidents of you shooting your sheep and adding the velocity of the bullet to the sheep’s velocity, pushing them directly into an enemy. Below is footage of what I have completed so far.

https://drive.google.com/file/d/1uTmM9crGmp_Z0nK4IobsTCbMvV4GidyG/view?usp=sharing
